...
<div class="table-container container-principal">
    <h2 style="text-align: center;">Gerenciar Instituições</h2>
    ...
    <div class="filtro-container" style="display: flex; flex-direction: column; align-items: center; gap: 15px; margin-bottom: 20px;">
        <form method="GET" action="" class="form-filtro" style="display: flex; gap: 10px;">
            <input type="text" name="filtro" placeholder="Pesquisar por Nome da Instituição" 
        ...
        <button class="btn-adicionar" onclick="openAddModal()" 
                style="background-color:#2118CD; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">Adicionar Nova Instituição</button>
    </div>
    ...
    $count_sql = "SELECT COUNT(*) as total FROM instituicao";
    if (!empty($filtro)) {
    ...
    // Exibir tabela
    if ($result->num_rows > 0) {
        echo "<table id='instituicaoTable' class='tabela-registros'>";
        echo "<thead><tr><th>Nome da Instituição</th><th>Ações</th></tr></thead>";
        echo "<tbody>";
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<button class='btn-excluir' onclick='openModal(\"instituicao.php?delete=" . $row['cod_instituicao'] . "\")'><i class='fas fa-trash-alt'></i></button>";
            echo "</td>";
            echo "</tr>";
        }
        echo "</tbody>";
