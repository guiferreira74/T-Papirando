...
<div class="table-container container-principal">
    <h2 style="text-align: center;">Gerenciar Bancas</h2>
    ...
    <div class="filtro-container" style="display: flex; flex-direction: column; align-items: center; gap: 15px; margin-bottom: 20px;">
        <form method="GET" action="" class="form-filtro" style="display: flex; gap: 10px;">
            <input type="text" name="filtro" placeholder="Pesquisar por Nome ou Link" 
        ...
        <button class="btn-adicionar" onclick="openAddModal()" 
                style="background-color:#2118CD; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">Adicionar Nova Banca</button>
    </div>
    ...
    $count_sql = "SELECT COUNT(*) as total FROM banca";
    if (!empty($filtro)) {
        $count_sql .= " WHERE 
            nome LIKE '%$filtro%' OR
            link LIKE '%$filtro%'
